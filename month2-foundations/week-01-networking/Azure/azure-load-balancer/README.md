...
Hit a genuine outbound connectivity failure (`apt install nginx` timing out) caused by backend pool VMs on a Standard LB losing default outbound internet access. Diagnosed the cause and fixed it by adding an explicit 

**Outbound Rule** (`outbound-web`) with default SNAT port allocation — confirmed the fix by successfully installing nginx afterward.
